...
export class Message extends Model {
                        ...
                        <div class="_2jfIu">
                            <div class="_2cfqh">
                                <div class="_1QMEq _1kZiz fS1bA">
                                    ...
                                    <div class="_1_Gu6">
                                        <div class="message-audio-duration">00:00</div>
                                        <div class="_1sLSi">
	...
	static send(chatId, from, type, content) {
		return new Promise((resolve, reject) => {
			Message.getRef(chatId).add({
				...
				let msgRef = result.parent.doc(result.id);
				msgRef.set({
					status: 'sent'
				}, {
						merge: true
	...
	static sendDocument(chatId, from, file, filePreview, info) {
		Message.send(chatId, from, 'document', '').then(msgRef => {
			Message.upload(file, from).then((downloadURLFile) => {
				if (filePreview){
					Message.upload(filePreview, from).then(downloadURLFilePreview => {
						msgRef.set({
							content: downloadURLFile,
							preview: downloadURLFilePreview,
							filename: file.name,
							size: file.size,
							fileType: file.type,
							status: 'sent',
